...
export default function FiltersBar({ onCreate }: { onCreate: () => void }) {
  ...
  return (
    <div className="card p-3 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
      <div className="flex flex-1 flex-col gap-2 md:flex-row">
        ...
        />
        <select className="input md:max-w-xs" value={filters.assignee}
                onChange={(e) => setFilters({ assignee: e.target.value })}>
          <option value="">All assignees</option>
          {assignees.map(a => <option key={a} value={a}>{a}</option>)}
        </select>
        <select className="input md:max-w-xs" value={filters.tag}
